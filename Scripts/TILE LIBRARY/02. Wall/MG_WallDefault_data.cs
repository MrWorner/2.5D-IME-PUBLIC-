...
public class MG_WallDefault_data : MonoBehaviour
{
    [SerializeField]
    ...
    [SerializeField]
    private List<WallType> WallTypeList;
    ...
    private void CheckData()//Проверяем что стандартный ассет разработчика в целосности и ничего не забыто
    {
        ...
        if (TotalCountPropTile == 0)
            Debug.Log("<color=red>MG_WallDefault_data CheckData(): MG_FloorDefault_data НЕ ЗАПОЛНЕН РАЗРАБОТЧИКОМ!</color>");
        if (TotalCountPropTile != NameList.Count)
            Debug.Log("<color=red>MG_WallDefault_data CheckData(): NameList не равен количеству ID</color>");
        if (TotalCountPropTile != SpriteIsoHList.Count)
            Debug.Log("<color=red>MG_WallDefault_data CheckData(): SpriteIsoHList не равен количеству ID</color>");
        if (TotalCountPropTile != SpriteIsoVList.Count)
            Debug.Log("<color=red>MG_WallDefault_data CheckData(): SpriteIsoVList не равен количеству ID</color>");
        if (TotalCountPropTile != Sprite2DHList.Count)
            Debug.Log("<color=red>MG_WallDefault_data CheckData(): Sprite2DHList не равен количеству ID</color>");
        if (TotalCountPropTile != Sprite2DVList.Count)
            Debug.Log("<color=red>MG_WallDefault_data CheckData(): Sprite2DVList не равен количеству ID</color>");
    }
