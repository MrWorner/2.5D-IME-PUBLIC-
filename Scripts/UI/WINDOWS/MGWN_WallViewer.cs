...
public class MGWN_WallViewer : MGWN_PropertyViewer
{
    private Dictionary<WallType, MG_WinI_Prop> dic_chosenProp = new Dictionary<WallType, MG_WinI_Prop>(); //выбранные элементы каждой категории
    ...
    protected override void SendChosenToEditor(MG_WinI_Prop _winI_Prop)
    {
        ...
        editor.SetWallProperty(_prop);//передаем выбранное свойство в редактор. Сам редактор распознаст тип пола
    }
    ...
    protected override void Init()
    {
        ...
        MG_WinI_Prop _winI_Prop = ListItems[0];//берем первый элемент
        _winI_Prop.Click();//чтобы в начале была выбран, ОН ОБЯЗАН БЫТЬ ВЫБРАННЫМ
        //------InitDicTypeItem(WallType.SmallFence);//Добавить первый элемент для каждого типа
        SetBasicWall();//Установить БАЗОВУЮ стену для редактора
    }
    ...
    /// <summary>
    /// Установить БАЗОВУЮ стену для редактора
    /// </summary>
    private void SetBasicWall()
    {
        foreach (var _item in ListItems)//проходим по всем элементам в окне
        {
            MG_WallProperty _property = (MG_WallProperty) _item.GetProperty();//получаем свойство
            WallType _type = _property.Type;//вытаскиваем тип
            if (_type.Equals(WallType.Wall))//сравниваем
            {
                editor.SetBasicWallProperty(_property);//передаем редактору
                break;
            }
        }
    }
